...
Context:
Game: ${game.name}
Topic: "${userTopic || 'The Iron-Only Assembly (Total Consolidation)'}"
Template Archetype: "${templateType}"
User Key Facts: "${keyFacts || 'Optimal ratios, alternate recipe or factory math'}"

Return ONLY valid JSON matching this schema:
{
  "infographic_id": "ficsit-blueprint",
  "template_type": "${templateType}",
  "aspect_ratio": "9:16",
  "header": {
    "organization": "FICSIT INC.",
    "category": "LOGISTICS MASTERCLASS",
    "title": "THE IRON-ONLY ASSEMBLY",
    "subtitle": "(TOTAL CONSOLIDATION)"
  },
  "raw_inputs": [
    {
      "name": "Iron Ore",
  ...
  "pathway_nodes": [
    ...
    {
      "path_id": "Path B",
      "machine": "Smelter -> Constructor",
      "recipe_tag": "Iron Wire Recipe",
      "output_item": "Wire",
      "rate": "37.5/min",
      "is_bottleneck": false,
      "is_optimal": true
  ...
  "assembly_node": {
    "machine": "Assembler",
    "recipe_name": "Stitched Iron Plate Recipe",
    "final_output": "Reinforced Iron Plates",
    "output_rate": "5.625/min"
  },
  "math_callouts": [
    "Standard Cost: ~9 Iron Ingots per Reinforced Plate.",
    "Combo Cost: ~7 Iron Ingots per Reinforced Plate."
  ],
  "verdict": "ZERO COPPER. ZERO SCREWS. ULTIMATE EFFICIENCY."
}`;
  ...
  try {
    ...
    console.warn('Gemini synthesis fallback to default FICSIT spec payload:', err.message);
    return {
      infographic_id: 'ficsit-default',
      template_type: templateType,
      aspect_ratio: '9:16',
      header: {
        organization: 'FICSIT INC.',
        category: 'PRODUCTION GUIDE: ALTERNATE RECIPE',
        title: userTopic.toUpperCase() || 'THE IRON-ONLY ASSEMBLY',
        subtitle: '(TOTAL CONSOLIDATION)',
      },
      raw_inputs: [{ name: 'Iron Ore', rate: 'Sole Raw Input', icon_type: 'raw_ore' }],
      pathway_nodes: [
        { path_id: 'Path A', machine: 'Smelter', output_item: 'Iron Plate', rate: '18.75/min' },
        { path_id: 'Path B', machine: 'Smelter -> Constructor', recipe_tag: 'Iron Wire Recipe', output_item: 'Wire', rate: '37.5/min' },
      ],
      assembly_node: {
        machine: 'Assembler',
        recipe_name: 'Stitched Iron Plate Recipe',
...
export function generateFicsitSVG(data: FicsitInfographicData): string {
  ...
  <!-- Corner Bolts (Top-Left, Top-Right, Bottom-Left, Bottom-Right) -->
  ...
  <g transform="translate(55, 55)">
    ...
    <!-- Main Infographic Title (DIN / Barlow Compressed Style) -->
    <text x="0" y="115" fill="${C.TEXT_PRIMARY}" font-family="'Barlow Condensed', 'DIN Alternate', Impact, sans-serif" font-size="54" font-weight="900" letter-spacing="1">
    ...
    <g transform="translate(0, 135)">
      <text x="0" y="24" fill="${C.ACCENT_CYAN}" font-family="'Courier New', monospace, sans-serif" font-size="20" font-weight="bold" letter-spacing="1.5">
        /// ${escapeXml(data.header.subtitle || '(TOTAL CONSOLIDATION)')} ///
      </text>
    </g>
  </g>
  ...
  <!-- The Math Callout Card (Common to all templates) -->
  <g transform="translate(55, 1380)">
    ...
    <g transform="translate(30, 85)">
      ${data.math_callouts
        .map((line, idx) => `
      <g transform="translate(0, ${idx * 45})">
        <rect width="8" height="8" rx="2" fill="${C.ACCENT_CYAN}" y="-6"/>
        <text x="24" y="2" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', Roboto, sans-serif" font-size="20" font-weight="700">
          ${escapeXml(line)}
        </text>
      </g>`)
        .join('')}
  ...
  <!-- Bottom Executive Verdict Banner (Solid Industrial FICSIT Orange) -->
  <g transform="translate(55, 1640)">
    <rect width="970" height="130" rx="14" fill="url(#hazardGrad)" filter="url(#neonOrangeGlow)"/>
    <text x="485" y="75" text-anchor="middle" fill="#0a0e17" font-family="'Barlow Condensed', 'DIN Alternate', Impact, sans-serif" font-size="30" font-weight="900" letter-spacing="1.5">
      ${escapeXml(data.verdict || 'ZERO COPPER. ZERO SCREWS. ULTIMATE EFFICIENCY.')}
    </text>
  </g>
...
function renderComparisonTemplate(data: FicsitInfographicData, C: typeof FICSIT_COLORS): string {
  ...
  <g transform="translate(55, 270)">
    ...
    <g transform="translate(40, 100)">
      ...
      <text x="20" y="40" fill="${C.TEXT_MUTED}" font-family="monospace" font-size="13" font-weight="bold">INPUT</text>
      <text x="20" y="80" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', sans-serif" font-size="20" font-weight="bold">${escapeXml(data.raw_inputs[0]?.name || 'Raw Ore')}</text>
      <text x="20" y="115" fill="${C.ACCENT_CYAN}" font-family="monospace" font-size="15" font-weight="bold">${escapeXml(data.raw_inputs[0]?.rate || '18.75/min')}</text>
      ...
      <text x="360" y="40" fill="${C.TEXT_MUTED}" font-family="monospace" font-size="13" font-weight="bold">MACHINE</text>
      <text x="360" y="85" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', sans-serif" font-size="20" font-weight="bold">${escapeXml(pA.machine)}</text>
      ...
      <text x="700" y="115" fill="${C.STATUS_WARNING}" font-family="monospace" font-size="15" font-weight="bold">${escapeXml(pA.rate)}</text>
    </g>

    <text x="40" y="340" fill="${C.TEXT_MUTED}" font-family="'Segoe UI', sans-serif" font-size="16">
      Standard Logistics Note: High machine footprint, redundant multi-step rod pipeline.
    </text>
  </g>
  ...
  <g transform="translate(55, 820)">
    ...
    <g transform="translate(40, 100)">
      ...
      <text x="20" y="85" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', sans-serif" font-size="20" font-weight="bold">${escapeXml(data.raw_inputs[0]?.name || 'Raw Ore')}</text>
      <text x="20" y="125" fill="${C.STATUS_OPTIMAL}" font-family="monospace" font-size="16" font-weight="bold">DIRECT INFEED</text>
      ...
      <text x="360" y="85" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', sans-serif" font-size="20" font-weight="bold">${escapeXml(pB.machine)}</text>
      ...
      <text x="700" y="85" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', sans-serif" font-size="20" font-weight="bold">${escapeXml(pB.output_item)}</text>
      <text x="700" y="125" fill="${C.STATUS_OPTIMAL}" font-family="monospace" font-size="17" font-weight="900">${escapeXml(pB.rate)}</text>
    </g>

    <text x="40" y="360" fill="${C.ACCENT_CYAN}" font-family="'Segoe UI', sans-serif" font-size="17" font-weight="700">
      ✓ Alternate Efficiency Verdict: Cuts constructor buildings by 50%, eliminates copper requirement completely.
    </text>
  </g>`;
}
...
function renderProblemSolutionTemplate(data: FicsitInfographicData, C: typeof FICSIT_COLORS): string {
  ...
  <g transform="translate(55, 270)">
    ...
    <g transform="translate(40, 110)">
      ...
      <text x="25" y="90" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', sans-serif" font-size="24" font-weight="bold">${escapeXml(data.raw_inputs[0]?.name || 'Raw Ore')}</text>
      <text x="25" y="135" fill="${C.STATUS_WARNING}" font-family="monospace" font-size="16">Bottleneck: High Belt Density</text>
      ...
      <text x="25" y="90" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', sans-serif" font-size="24" font-weight="bold">${escapeXml(pA.output_item)}</text>
      <text x="25" y="135" fill="${C.STATUS_WARNING}" font-family="monospace" font-size="16">Yield: ${escapeXml(pA.rate)}</text>
    </g>

    <text x="40" y="380" fill="${C.STATUS_WARNING}" font-family="'Segoe UI', sans-serif" font-size="18" font-weight="700">
      ⚠ Bottleneck Detected: Excessive intermediate processing stages and belt congestion.
    </text>
  </g>
  ...
  <g transform="translate(55, 820)">
    ...
    <g transform="translate(40, 110)">
      ...
      <text x="25" y="45" fill="${C.ACCENT_CYAN}" font-family="monospace" font-size="14">OPTIMAL RECIPE INFEED</text>
      <text x="25" y="90" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', sans-serif" font-size="24" font-weight="bold">${escapeXml(data.raw_inputs[0]?.name || 'Raw Ore')}</text>
      <text x="25" y="135" fill="${C.STATUS_OPTIMAL}" font-family="monospace" font-size="16">Streamlined Direct Flow</text>
      ...
      <text x="25" y="90" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', sans-serif" font-size="24" font-weight="bold">${escapeXml(pB.output_item)}</text>
      <text x="25" y="135" fill="${C.STATUS_OPTIMAL}" font-family="monospace" font-size="16">Output: ${escapeXml(pB.rate)}</text>
    </g>

    <text x="40" y="380" fill="${C.STATUS_OPTIMAL}" font-family="'Segoe UI', sans-serif" font-size="18" font-weight="700">
      ✓ Resolution: Eradicates intermediate bottlenecks with higher material throughput.
    </text>
  </g>`;
}
...
function renderFlowchartConsolidationTemplate(data: FicsitInfographicData, C: typeof FICSIT_COLORS): string {
  const pA = data.pathway_nodes[0] || { machine: 'Smelter', output_item: 'Iron Plate', rate: '18.75/min' };
  const pB = data.pathway_nodes[1] || { machine: 'Smelter -> Constructor', output_item: 'Wire', rate: '37.5/min', recipe_tag: 'Iron Wire Recipe' };
  const ass = data.assembly_node || { machine: 'Assembler', recipe_name: 'Stitched Iron Plate', final_output: 'Reinforced Iron Plates', output_rate: '5.625/min' };
  ...
  <g transform="translate(55, 240)">
    ...
    <g transform="translate(300, 0)">
      <rect width="370" height="110" rx="12" fill="${C.BG_SURFACE_2}" stroke="${C.FICSIT_ORANGE}" stroke-width="2" filter="url(#neonOrangeGlow)"/>
      <text x="185" y="34" text-anchor="middle" fill="${C.ACCENT_CYAN}" font-family="monospace" font-size="13" font-weight="bold" letter-spacing="1">
        SOLE RAW INPUT
      </text>
      <text x="185" y="78" text-anchor="middle" fill="${C.TEXT_PRIMARY}" font-family="'Barlow Condensed', sans-serif" font-size="28" font-weight="900">
        ${escapeXml(data.raw_inputs[0]?.name || 'Iron Ore')}
      </text>

      <!-- Splitting Arrows (Y-Splitter) -->
      <path d="M 185 110 L 185 140 L 40 210" stroke="${C.ACCENT_CYAN}" stroke-width="3" fill="none"/>
      <polygon points="35,205 30,215 45,215" fill="${C.ACCENT_CYAN}"/>

      <path d="M 185 140 L 330 210" stroke="${C.ACCENT_CYAN}" stroke-width="3" fill="none"/>
      <polygon points="325,215 340,215 335,205" fill="${C.ACCENT_CYAN}"/>
    </g>
    ...
    <g transform="translate(20, 230)">
      ...
      <text x="25" y="125" fill="${C.TEXT_PRIMARY}" font-family="'Segoe UI', sans-serif" font-size="22" font-weight="bold">${escapeXml(pA.machine)}</text>
      ...
      <text x="25" y="235" fill="${C.FICSIT_ORANGE}" font-family="'Barlow Condensed', sans-serif" font-size="26" font-weight="900">${escapeXml(pA.output_item)}</text>
      <text x="395" y="235" text-anchor="end" fill="${C.TEXT_PRIMARY}" font-family="monospace" font-size="20" font-weight="bold">${escapeXml(pA.rate)}</text>
    </g>
    ...
    <g transform="translate(530, 230)">
      ...
      ${pB.recipe_tag ? `
      <rect x="160" y="20" width="180" height="28" rx="6" fill="rgba(34, 197, 94, 0.2)" stroke="${C.STATUS_OPTIMAL}" stroke-width="1.5"/>
      <text x="250" y="39" text-anchor="middle" fill="${C.STATUS_OPTIMAL}" font-family="monospace" font-size="12" font-weight="bold">${escapeXml(pB.recipe_tag)}</text>
      ` : ''}
      ...
      <text x="25" y="195" fill="${C.TEXT_MUTED}" font-family="monospace" font-size="14">BRANCH OUTPUT</text>
      <text x="25" y="235" fill="${C.FICSIT_ORANGE}" font-family="'Barlow Condensed', sans-serif" font-size="26" font-weight="900">${escapeXml(pB.output_item)}</text>
      <text x="395" y="235" text-anchor="end" fill="${C.TEXT_PRIMARY}" font-family="monospace" font-size="20" font-weight="bold">${escapeXml(pB.rate)}</text>
    </g>
    ...
    <g transform="translate(185, 590)">
      ...
      <text x="25" y="130" fill="${C.TEXT_PRIMARY}" font-family="'Barlow Condensed', sans-serif" font-size="30" font-weight="900">${escapeXml(ass.machine)}</text>
    ...
    <!-- Final Output Node (Reinforced Iron Plates) -->
    <g transform="translate(185, 900)">
      ...
      <text x="25" y="95" fill="${C.TEXT_PRIMARY}" font-family="'Barlow Condensed', sans-serif" font-size="36" font-weight="900">
        ${escapeXml(ass.final_output)}
      </text>
      <text x="25" y="138" fill="${C.STATUS_OPTIMAL}" font-family="monospace" font-size="22" font-weight="900">
